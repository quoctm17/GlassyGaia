...
export function registerItemRoutes(router) {
  ...
  router.delete('/items/:slug/episodes/:episode', async (request, env) => {
    ...
    try {
      ...
      try {
        if (cardIds.length) {
          // D1 limit: max 100 bound parameters per query. Batch card_ids.
          const MAX_BIND = 100;
          for (let i = 0; i < cardIds.length; i += MAX_BIND) {
            const chunk = cardIds.slice(i, i + MAX_BIND);
            const ph = chunk.map(() => '?').join(',');
            try { await env.DB.prepare(`DELETE FROM card_subtitles WHERE card_id IN (${ph})`).bind(...chunk).run(); } catch { }
            try { await env.DB.prepare(`DELETE FROM card_subtitles_fts WHERE card_id IN (${ph})`).bind(...chunk).run(); } catch { }
            try { await env.DB.prepare(`DELETE FROM card_difficulty_levels WHERE card_id IN (${ph})`).bind(...chunk).run(); } catch { }
          }
  ...
  router.delete('/items/:slug', async (request, env) => {
    const filmSlug = decodeURIComponent(request.params.slug);
    const result = await deleteContentItemBySlug(env, filmSlug);
