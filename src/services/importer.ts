...
function detectMappingFromHeaders(headers: string[]): { mapping: ColumnMapping; detectedLangs: string[]; primary?: string } {
  ...
  const difficultyScore = headerOf("difficulty", "difficulty_score", "diff", "card_difficulty");
  ...
  for (const h of headers) {
    ...
    if (canon && ["en","vi","zh","ja","ko","id","th","ms","zh_trad"].includes(canon)) {
  ...
  const knownLangs = new Set(["en","vi","zh","ja","ko","id","th","ms","zh_trad"]);
  for (const original of headers) {
    ...
    if ([start, end, type, sentence, cefr, jlpt, hsk, difficultyScore].filter(Boolean).includes(raw)) continue;
  ...
  return { mapping: { start, end, type, sentence, cefr, jlpt, hsk, difficultyScore, frameworkCols, subtitles }, detectedLangs: detected, primary };
}
...
export async function importFilmFromCsv(opts: ImportOptions, onProgress?: (done: number, total: number) => void) {
  ...
  const cards = rows.map((row, index) => {
    ...
    const episodeSlug = `e${episodeNum}`;
  ...
  const difficulty_levels: Array<{ framework: string; level: string; language?: string }> = [];
    ...
    const difficultyScoreNum = difficultyScoreVal ? Number(difficultyScoreVal) : undefined;
    ...
    return {
      ...
      film_id: filmSlug,
      subtitle,
      type: mapping.type ? (row[mapping.type] || "").toString().trim() : undefined,
      sentence: mapping.sentence ? (row[mapping.sentence] || "").toString().trim() : undefined,
      ...
      difficulty: Number.isFinite(difficultyScoreNum) ? difficultyScoreNum : undefined,
      image_url,
      audio_url,
    };
  });
